function result = simulate_im_over_regular_graph(U, graph, alpha, iteration_time, N)
    ...
    for i=1:N                           %define the strategy state 1,2
        if i<=K(1)
    ...
    for time=2:iteration_time
        ...
        for p = 1:N          %每次遍历全部点，并根据IM——rule更新
            ...
            ss=strategy_state(i);
            fit_self = fitness_calculate(i,KK(i));    %算出自己的fitness
            fit=[0,0];
            for j=1:KK(i)
